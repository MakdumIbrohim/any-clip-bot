...
import { availableHeights, estimateAudioSize, estimateVideoSize, formatBytes, runDownload } from "../src/downloader.js";
...
console.log("duration:", info.duration, "s | uploader:", info.uploader, "| thumb:", !!info.thumbnail);
...
if (process.argv[3]?.startsWith("download")) {
  const kind = process.argv[3] === "download"
    ? { type: "audio" as const }
    : { type: "video" as const, height: Number(process.argv[3].split(":")[1]) };
  const job = {
    id: "smoke",
    chatId: 0,
    userId: 0,
  ...
  const res = await runDownload(job, (p, stage) => console.log(`[${stage}] ${p ?? ""}%`));
  console.log("result:", res.ok ? `OK ${formatBytes(res.size)} -> ${res.path}` : res);
  if (res.ok) fs.rmSync(path.join(config.tmpDir, "smoke"), { recursive: true, force: true });
}
